...
class AddChild extends PureComponent {
  ...
  showConfirm() {
    ...
    if (this.state.hasHouse === "是") {
      if (!this.state.houseOwner || !this.state.houseNumbers) {
        message.warning("请填写完整的房产居住信息！");
        return;
      }
    } else {
    ...
    if (this.state.isChecked) {
      ...
      message.warning("请阅读并勾选承诺书后再进行提交！");
    }
  }
  //保存Modal
  handleSave() {
  ...
  commit(commit) {
    ...
    http.post(api.SAVEANDCOMMIT, params).then((resp) => {
      if (resp.success) {
        if (commit) {
          this.setState({
            forecastCode: resp.data.forecastCode,
            isShowSuccess: true,
          });
        } else {
          message.success(commit ? "提交成功" : "保存成功");
          this.props.history.push({
            pathname: "/patriarch/childManage",
          });
        }
      } else {
    ...
    return (
      <AddChildWrapper>
              ...
              <ReportInfo>
                ...
                <div className="report-text">
                  ...
                  <Checkbox
                    ...
                    disabled={isDisabled}
                    onChange={this.isCheckedChange.bind(this)}
                  >
                    <span>我已阅读并接受</span>
                  </Checkbox>
                </div>
        ...
        <Bottom>
          {!isDisabled ? (
            <Fragment>
              <div className="save-button" onClick={this.showSave.bind(this)}>
                保存
              </div>
              <div
